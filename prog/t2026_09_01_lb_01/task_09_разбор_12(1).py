...
last_numbers_for_rectangles = filter_rectangles.apply(lambda row: row[~row.duplicated(keep = False)].values[0],axis = 1)

print(last_numbers_for_squares.iloc[:].sum() + last_numbers_for_rectangles.iloc[:].sum())
